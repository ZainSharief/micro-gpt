...
def train(args):
    ...
    if args.mode == 'pretrain':
        ...
        checkpoint = torch.load('weights/base_model_checkpoint_190000.pth', map_location='cpu')
    ...
    if args.mode != 'pretrain' and args.checkpoint_path: # only pretrain will start without a checkpoint
    ...
    #scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer,max_lr=args.max_lr, total_steps=total_steps*args.epochs, pct_start=0.15, anneal_strategy='cos')
    ...
    for epoch in range(args.epochs):
        ...
        for current_batch, (xb, yb, mask) in enumerate(dataloader):
            ...
            if args.mode == 'pretrain' and (current_batch + 1) % args.save_iter == 0:
                save_checkpoint(model, optimizer, None, scaler, f'weights/base_model_checkpoint_{current_batch+1}.pth')
                total_loss = 0.0
                counter = 0
        ...
        if val_dataset:
            ...
            if val_loss <= min_val_loss:
                ...
                save_checkpoint(model, optimizer, None, scaler, f'weights/reward_checkpoint_{epoch+1}.pth')
